...
// `svcHasLocalHostNetEndPnt` is true if this service has at least one host-networked endpoint that is local to this node
// `isETPLocal` is true if the svc.Spec.ExternalTrafficPolicy=Local
func getExternalIPTRules(svcPort kapi.ServicePort, externalIP, dstIP, intf string, svcHasLocalHostNetEndPnt, isETPLocal bool) []iptRule {
	...
	// FEDE HACK: we must force the port because when dealing with vrfs we would end up with duplicate
	// rules
	// -A OVN-KUBE-EXTERNALIP -d 192.168.10.1/32 -p tcp -m tcp --dport 80 -j DNAT --to-destination 10.96.97.72:80
	// -A OVN-KUBE-EXTERNALIP -d 192.168.10.1/32 -p tcp -m tcp --dport 80 -j DNAT --to-destination 10.96.243.11:80
		...
		for _, externalIP := range externalIPs {
			...
			if clusterIP, err := util.MatchIPStringFamily(utilnet.IsIPv6String(externalIP), clusterIPs); err == nil {
				if svcTypeIsETPLocal && !svcHasLocalHostNetEndPnt {
					...
					rules = append(rules, getExternalIPTRules(svcPort, externalIP, "", intf, svcHasLocalHostNetEndPnt, svcTypeIsETPLocal)...)
				}
				// case2 (see function description for details)
				rules = append(rules, getExternalIPTRules(svcPort, externalIP, clusterIP, intf, svcHasLocalHostNetEndPnt, false)...)
			}
		}
